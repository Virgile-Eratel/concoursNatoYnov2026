...
| Installeur `Facom_ScanDiag_setup.exe` | Protocole logiciel côté PC (Bluetooth SPP) |
...
| **Micro-caméra** | **CMOS WXGA, 1 Mégapixel, 30 fps** | Technische Daten |
...
| 4 | **Micro-caméra CMOS** | WXGA 1 MP, 30 fps, interface parallèle/MIPI 🔧 | Capteur CMOS (réf. PCB) 🔧 |
...
- **MCU / SoC principal** : doit gérer caméra + laser + BT + USB → probablement un MCU ARM
  Cortex-M avec interface caméra (DCMI) ou un SoC dédié. *Réf. à relever.*
...
flowchart TB
    ...
    subgraph CAPTEURS["🎯 Chaîne de mesure optique"]
        ...
        CAM[Caméra CMOS<br/>WXGA 1MP 30fps]
    end
...
- **Caméra 1 MP / 30 fps** : résolution modeste, optique fixe orientée mesure rapprochée.
...
- [ ] Référence **capteur CMOS** + interface (DCMI parallèle / MIPI)
